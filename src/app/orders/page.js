...
export default function Orders() {
    ...
    return (
        <div className="w-full max-w-full h-screen flex flex-col justify-center items-center mx-auto px-4">
            <div className="poppins w-4/5 h-3/4">
                <div className="relative h-full bg-[#001a8e] rounded-l-xl! z-10">
                    ...
                    <div className="absolute inset-y-0 right-0 h-full w-3/4">
                        <div className="relative h-full w-full grid grid-cols-3 grid-rows-3 items-stretch">
                            {Object.entries(kpiConfig).map(([key, config], idx) => {
                                ...
                                return (
                                    <div
                                        key={key}
                                        className={`relative group overflow-hidden ${idx % 3 === 0 ? "rounded-l-2xl" : "border-l-0"} ${idx < 3 ? "border-t" : ""} ${idx === 3 ? "" : ""} h-full w-full bg-zinc-50 border-l border-b border-r border-[#001a8e] pt-2 pl-2 flex flex-col justify-between`}
                                    >
                                        <button
                                            type="button"
                                            className="absolute right-3 top-2 -translate-y-10 group-hover:translate-y-0 transition-transform duration-300 ease-in-out"
                                        >
