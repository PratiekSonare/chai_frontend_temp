...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    const growthData = payload.find(
      (p) => p.payload && p.payload.growth !== undefined,
    );
    const styleNameData = payload[0]?.payload?.style_name;

    return (
      <div
        className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl p-5 backdrop-blur-xl"
      ...
      >
        ...
        {growthData &&
          growthData.payload &&
          growthData.payload.growth !== null && (
    ...
    <ResponsiveContainer width="100%" height="100%">
      <ScatterChart margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
        ...
        <Scatter
          ...
          data={data}
          fill={CHART_COLORS[0]}
          label={{ dataKey: "sku", fill: "#000", fontSize: 12, dy: -20 }}
        />
      </ScatterChart>
    </ResponsiveContainer>
  </div>
  ...
  return (
    <div className="h-64 flex items-center justify-center p-6grid grid-cols-2 gap-4">
      {Object.entries(data).map(([key, value]) => {
        const { key: displayKey, value: displayValue } = formatStatValue(
          key,
          value,
        );
        return (
          <div
            key={key}
            className="flex flex-col items-center justify-center p-5 rounded-lg bg-transparent border border-blue-500 dark:from-blue-900/20 dark:to-indigo-900/20 dark:border-blue-800"
          >
