...
function summarizeCard(routeId, data) {
  ...
  if (routeId === "cod-prepaid") {
    ...
    return [
      { label: "COD Orders", value: numberFmt.format(cod.order_count || 0) },
      {
        ...
        value: numberFmt.format(prepaid.order_count || 0),
      },
      { label: "COD Revenue", value: currencyFmt.format(cod.revenue || 0) },
      {
        label: "Prepaid Revenue",
        value: currencyFmt.format(prepaid.revenue || 0),
      },
    ];
  }
  ...
  if (routeId === "top-states") {
    ...
    return [
      ...
      {
        label: "Top Growth",
        value: `${Number(top.growth_pct || 0).toFixed(2)}%`,
      },
      {
        label: "Rest Revenue",
        value: currencyFmt.format(rest.current_revenue || 0),
      },
      {
        label: "Rest Share",
        value: `${Number(rest.revenue_share || 0).toFixed(2)}%`,
      },
    ];
  }
  ...
  if (routeId === "size-mix") {
    ...
    return [
      ...
      {
        label: "Current Units",
        value: numberFmt.format(size.current_units || 0),
      },
      {
        label: "Share Change",
        value: `${Number(size.share_change_pp || 0).toFixed(2)} pp`,
      },
    ...
    <div className="w-[90%] h-screen flex flex-col justify-center items-start gap-10">
      <div className="flex flex-col items-start text-left gap-1">
        ...
        <span className="poppins text-lg text-gray-500">
          Live comparisons from history routes with auto-rotating summary cards.
        </span>
      </div>
        ...
        <div className="relative h-full bg-[#001a8e] rounded-xl flex flex-col p-5 gap-5 overflow-hidden">
          ...
          >
            <CarouselContent className="h-full">
              {(results.length > 0 ? results : COMPARISON_ROUTES).map(
                (route) => {
                  ...
                  return (
                    ...
                    >
                      <article className="h-full rounded-xl border border-slate-200 bg-slate-50 p-5 flex flex-col gap-4">
                        <header
                          className={`rounded-lg bg-linear-to-r ${route.accent} px-3 py-2 text-white`}
                        >
                          <h3 className="oswald text-lg tracking-wide">
                            {route.title}
                          </h3>
                        ...
                        <div className="mt-auto flex items-center justify-between text-xs">
                          <span
                            className={`font-semibold ${hasError ? "text-rose-600" : "text-emerald-600"}`}
                          >
