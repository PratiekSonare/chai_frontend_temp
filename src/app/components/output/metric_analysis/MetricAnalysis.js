...
export default function MetricAnalysis({
  ...
  const renderMetricContent = (value, metricKey) => {
    ...
    const renderNestedObject = (obj, level = 0) => {
      return Object.entries(obj).map(([key, val]) => (
        <div
          key={key}
          className={`${level === 0 ? "mb-2" : "mb-0"} ${level > 0 ? "px-3 flex flex-row justify-between" : ""}`}
        >
          <div
            className={`!w-full uppercase font-semibold !text-[#001FB0] ${level === 0 ? "border-b border-blue-200 pb-1 text-lg" : "md"}`}
          >
            {key.replace(/_/g, " ")}
          </div>
          {val !== null && typeof val === "object" ? (
            <div className="mt-1">{renderNestedObject(val, level + 1)}</div>
          ) : (
  ...
  if (Array.isArray(metric_calculated)) {
    ...
    metricsArray = metric_calculated.map((m) => [
      m.metric_name || m.name,
      m.value,
    ]);
  } else {
    // Old format: object
    const metrics = metric_calculated?.metrics
      ? metric_calculated.metrics
      : metric_calculated;
  ...
  return (
    <div
      className={`max-w-7xl grid ${showInsight ? "grid-cols-3" : "grid-cols-1"} gap-4`}
    >
      {showInsight && (
        <div className="col-start-1 col-span-2">
          <div
            className="pointer-events-auto select-none relative border rounded-xl w-full !h-fit"
                ...
                >
                  <div className="flex flex-col !gap-0">
                    ...
                    <span className="poppins text-xs italic text-gray-300">
                      Comprehensive analysis of calculated metrics with detailed
            ...
            <div className="rounded-b-xl p-0 h-96 overflow-y-scroll bg-gray-50 poppins">
              <ReactMarkdown
                components={{
                  // Style bullet points
                  ul: ({ children }) => (
                    <ul className="space-y-2 !pl-0">{children}</ul>
                  ),
        ...
        >
          <CarouselContent className="!h-full">
            {metricsArray.length > 0 ? (
              metricsArray.map(([metricKey, metricValue]) => {
                const metricTitle =
                  TOOL_TITLES[metricKey] ||
                  metricKey.replace(/_/g, " ").toUpperCase();
                return (
                  <CarouselItem key={metricKey} className="basis-full">
                    <div
                      className="pointer-events-auto select-none relative w-full rounded-xl !text-[#001FB0] border-4 border-[#001FB0] h-full"
                      onClick={() => setIsOpen(false)}
                    >
                      <div
                        onClick={(e) => {
                        ...
                        className="!border-b-4 !border-[#001FB0] flex flex-row items-center justify-between rounded-t-xl h-fit cursor-pointer"
                      >
                        <span className="block text-lg py-2 px-4 rounded-t-xl oswald uppercase">
                          {metricTitle}
                        </span>
                        {isOpen && (
                          <div
                          ...
                          >
                            <div className="flex flex-col !gap-0">
                              ...
                              <span className="poppins text-xs italic text-gray-300">
                                {metricTitle} - Calculated metric value showing
              ...
              <CarouselItem className="basis-full">
                ...
                >
                  <div className="flex flex-row items-center justify-between rounded-t-xl h-fit cursor-pointer">
                    <span className="block text-md py-2 px-4 rounded-t-xl oswald uppercase">
                      NO METRIC
                    </span>
                    <svg
                      className={`h-4 px-4 transition-transform duration-200 ease-in`}
