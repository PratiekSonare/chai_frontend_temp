...
export default function Comparison({ isSuccess, searchData, searchType, searchFilter, comparisonType, detailedMetrics, metricsLoading, createPaymentChart, refreshKey }) {
        ...
        if (isSuccess && searchType === "comparison" && searchData?.comparison_data) {
            if (comparisonType === "multi_group") {
                ...
                if (searchData.comparison_data.group_summaries) {
                    ...
                    return Object.entries(searchData.comparison_data.group_summaries).map(([groupName, groupData], index) => ({
                ...
                if (searchFilter === "state") {
                    ...
                    if (detailedMetrics) {
                        ...
                        groupKeys.forEach((groupKey, index) => {
                            ...
                            if (groupMetrics?.top_states && Object.keys(groupMetrics.top_states).length > 0) {
                                Object.entries(groupMetrics.top_states).forEach(([state, count]) => {
                        ...
                        const result = Array.from(stateDataMap.entries()).map(([stateName, groupData]) => {
                        ...
                        if (result.length === 0 || result.every(item => item.value === 0)) {
                            const groupComparison = groupKeys.map((groupKey, index) => {
                                const groupMetrics = detailedMetrics[groupKey];
                                const totalCount = groupMetrics?.total_orders ||
    ...
    }, [isSuccess, searchType, searchData, searchFilter, detailedMetrics, comparisonType]);
