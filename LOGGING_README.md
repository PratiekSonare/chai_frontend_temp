...
2. **New Endpoints**:
   - `ws://${process.env.NEXT_PUBLIC_API_URL}/ws/logs` - WebSocket for real-time log streaming
...
```javascript
const ws = new WebSocket('ws://${process.env.NEXT_PUBLIC_API_URL}/ws/logs');
...
const response = await fetch('https://${process.env.NEXT_PUBLIC_API_URL}/query', {
  method: 'POST',
...
const logsResponse = await fetch(`https://${process.env.NEXT_PUBLIC_API_URL}/logs/${requestId}`);
...
const eventSource = new EventSource('https://${process.env.NEXT_PUBLIC_API_URL}/logs/stream');
